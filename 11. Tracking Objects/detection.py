...
def image2tensor(image):
    ...
    image = resize(image, (300, 300), preserve_range=True)  # resize image to 300x300
    ...
    tensor = torch.tensor(image.copy(), dtype=torch.float32).unsqueeze(0)
...
def extract_detections(frame, min_confidence=0.6, labels=None):
    ...
    detections = np.zeros((results.shape[0], 5), dtype=np.float32)
    detections[:, 0] = results[:, 0]
    detections[:, 1] = results[:, 2]
    detections[:, 2] = results[:, 3]
    detections[:, 3] = results[:, 4]  
    detections[:, 4] = results[:, 5]
...
def draw_detections(frame, detections):
    ...
    for det in detections:
        label_id, x_min, y_min, x_max, y_max = det
        color = get_color(label_id)

        ll, rr = rectangle(frame.shape, (y_min, x_min), (y_max, x_max))
        frame[ll, rr] = color
