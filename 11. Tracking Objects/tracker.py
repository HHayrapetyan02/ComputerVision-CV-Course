...
class Tracker:
    ...
    def prev_detections(self):
        ...
        detections = []
        # Write code here
        start_index = max(0, len(self.detection_history) - self.lookup_tail_size)

        latest_detections = {}
        for i in range(len(self.detection_history) - 1, start_index - 1, -1):
            frame_detections = self.detection_history[i]
            for detection in frame_detections:
                tracklet_id = detection[0]
                if tracklet_id not in latest_detections:
                    latest_detections[tracklet_id] = detection
        
        detections = list(latest_detections.values())
        return detection_cast(detections)
    ...
    def bind_tracklet(self, detections):
        ...
        for i, det in enumerate(detections):
            for j, prev_det in enumerate(prev_detections):
                iou_matrix[i][j] = iou_score(det[1:], prev_det[1:])
        ...
        for _, det_idx, prev_idx in matches:
            if det_idx not in used_detections and prev_idx not in used_prev_detections:
                detections[det_idx, 0] = prev_detections[prev_idx, 0]
                used_detections.add(det_idx)
                used_prev_detections.add(prev_idx)
